...
4. [Décision retenue](#4-décision-retenue)
5. [Conséquences](#5-conséquences)
6. [À surveiller post-décision](#6-à-surveiller-post-décision)
...
Mardi 30 juin 2026 à 10h00, la perturbation J2 est révélée : la latence de génération d'un quiz de 10 QCM est jugée inacceptable.

**Mesures effectuées par l'équipe** (script `benchmark.sh`, 5 runs, environnement Sprint 1) :
...
| Ressenti utilisateur (3 bêta-testeurs) | « beaucoup trop long, j'abandonne » |
| Taux de complétion quiz | -34% vs session de référence |
...
> **Objectif.** Lister exhaustivement les alternatives évaluées, y compris celles écartées rapidement.

| Option | Avantages | Inconvénients | Coût (effort/risque) |
|---|---|---|---|
| **A. Statu quo** (Llama 3.1 8B) | 0 effort, qualité maintenue (8,2/10) | 42,3 s latence inacceptable, MVP risque rejet | Faible effort / **risque élevé** sur démo PO |
...
| **C. Migrer vers Llama 3.2 3B** ✅ | Latence **12,4 s p50** (-71%), RAM -5 GB, même famille Meta (prompts compatibles), qualité 8,0/10 (-2%) | Qualité légèrement inférieure au 8B, mais acceptable pour QCM pédagogiques | Effort **FAIBLE** (config Ollama 30 min) / risque qualité **MINIME** |
| **D. Migrer vers Phi-3-mini (3,8B)** | Latence 8,2 s p50 (-81%), RAM -6 GB | Qualité 6,9/10 (-16%), 4 questions ambiguës/50 vs 1/50, famille Microsoft (moins de cohérence prompts) | Faible effort / risque qualité **ÉLEVÉ** |

---

## 4. Décision retenue

> **Décision : Option C — Migration vers Llama 3.2 3B** comme modèle LLM par défaut, avec feature flag pour rollback rapide vers Llama 3.1 8B si la qualité dégrade en production.

### 4.1. Justification du choix

L'option C offre le **meilleur compromis qualité/latence** parmi les options conformes à la stack imposée :
...
Options écartées :
- **Option D (Phi-3-mini)** : perte de qualité trop importante (-16%) et famille Microsoft moins cohérente
- **Option cloud** : violation RGPD, non négociable pour la cible B2B éducation
...
| RAM serveur | référence | -5 GB libérés | — |
| Coût électrique serveur | référence | estimé -35% | — |
| Compatibilité prompts | — | ✅ Même famille Meta | Aucune réécriture |
| Démo PO Sprint | risque rejet | ✅ Sécurisée (< 15 s) | — |

### 5.2. Conséquences négatives (coûts assumés)

| Métrique | Avant | Après | Impact |
|---|---|---|---|
| Qualité moyenne quiz | 8,2/10 | 8,0/10 | -2% (acceptable) |
| Questions ambiguës | 1/50 | 2/50 | ×2 — nécessite validation renforcée |
| Dépendance validation post-LLM | — | Point de défaillance unique si bug parser | Risque à surveiller |
| Risque sunset modèle | — | Llama 3.2 3B potentiellement déprécié par Meta | Veille active requise |

---

## 6. À surveiller post-décision

> **Objectif.** Définir les indicateurs permettant de juger si la décision était bonne, et les signaux déclenchant un nouvel ADR.
...
> **Date de revue planifiée de cet ADR** : 2 semaines après mise en production (soit semaine S+2 post-MVP), avec PO et équipe IA. Une re-décision implique un nouvel ADR-XXXX qui supersede celui-ci.
...
|---|---|---|
| L'ADR a un numéro unique (ADR-XXXX) et un statut explicite | ✅ Oui | ADR-0002, statut Accepté. Remplace ADR-0001. |
| Le contexte décrit la situation factuelle avec des chiffres mesurés | ✅ Oui | Latence p50 = 42,3 s, p95 = 51,2 s — 5 runs via script |
| Au moins 3 options listées dans le tableau comparatif, incluant « ne rien faire » | ✅ Oui | 4 options (A à D), incluant le statu quo et Phi-3-mini |
| La décision retenue est annoncée en 1 phrase claire | ✅ Oui | Option C clairement identifiée et nommée (Llama 3.2 3B) |
| La justification s'appuie sur les critères du contexte (latence, qualité, contrainte stack) | ✅ Oui | Compromis idéal qualité (-2%) / latence (-71%) |
| Les mesures de mitigation des inconvénients sont listées explicitement | ✅ Oui | 4 mesures : feature flag, parsing JSON strict, audit hebdo, veille technique |
| Les conséquences positives ET négatives sont chiffrées | ✅ Oui | +71% latence (gain), -2% qualité (perte assumée), -5 GB RAM libérée |
| Au moins 3 KPIs définis avec seuil cible + seuil d'alerte + action | ✅ Oui | 5 KPIs définis avec seuils d'action clairs |
| Une date de revue de l'ADR est fixée (2-4 semaines après décision) | ✅ Oui | Planifiée à S+2 post-MVP |
...
- Michael Nygard, *Documenting Architecture Decisions* (article fondateur, 2011) : cognitect.com/blog/2011/11/15/documenting-architecture-decisions
- ADR GitHub, Collection de modèles et exemples : github.com/joelparkerhenderson/architecture-decision-record
- ThoughtWorks Tech Radar, ADR « Adopt » : thoughtworks.com/en-us/radar/techniques/lightweight-architecture-decision-records
- APOCAL'IPSSI, Perturbation J2 (la latence inacceptable) : mohamedelafrit.com/teaching/APOCALIPSSI/pages/perturbations/j2-technique.php

### Convention de versionnement

- `v1.0` — version initiale produite lors de la perturbation J2
- `v1.1`, `v1.2` — révisions mineures (typo, ajout d'item) après revue PO
- `v2.0` — révision majeure suite à une nouvelle perturbation
